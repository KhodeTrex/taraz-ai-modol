  ...
  return (
    <GlassCard className="w-full max-w-3xl mx-auto flex flex-col h-[90vh] transition-all duration-500">
      <div className="flex justify-between items-center pb-4 border-b-2 border-white/50">
        <div>
            <h1 className="text-2xl font-bold text-sky-800">چت با هوش مصنوعی</h1>
      ...
      <div className="flex-grow overflow-y-auto my-4 pl-4 space-y-4">
        {messages.map((msg) => (
          ...
          >
            {msg.sender === MessageSender.AI && <div className="w-8 h-8 rounded-full bg-sky-500 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">AI</div>}
            <div
        ...
        {isLoading && (
            <div className="flex items-end gap-3 justify-start">
                <div className="w-8 h-8 rounded-full bg-sky-500 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">AI</div>
      ...
      <div className="mt-auto flex gap-4">
        <input
          ...
          className="flex-grow px-4 py-3 bg-white/70 rounded-lg border border-transparent focus:outline-none focus:ring-2 focus:ring-sky-500 transition"
          disabled={isLoading}
        />
        <button
          onClick={handleSend}
          disabled={isLoading}
          className="bg-sky-500 text-white font-bold py-3 px-6 rounded-lg hover:bg-sky-600 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-opacity-50 transition disabled:bg-sky-300"
        >
          ارسال
        </button>
      </div>
